...
{
    elink_rs_idle        = 0x00,
    elink_rs_rx_pre      = 0x01,
    elink_rs_rx_sfd      = 0x02,
    elink_rs_rx_len_h    = 0x03,
    elink_rs_rx_len_l    = 0x04,
    elink_rs_rx_data     = 0x05,
    ...
    do
    {
        ...
        switch(rd_run_s)
        {
            ...
            case elink_rs_tx_data:
            {
                ...
                {
                    mix_elink_oper->elink_link_run_s = elink_rs_rx_pre; //回到接收状态
                }
                else
                {//转移数据到PHY
